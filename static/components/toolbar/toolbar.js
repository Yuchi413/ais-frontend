...
function parseAndDisplayCSV(csvData) {
    ...
    const pois = rows.slice(1);
    pois.forEach(row => {
        const [name, longitude, latitude, description] = row.split(',');

        if (name && latitude && longitude) {
            const position = Cesium.Cartesian3.fromDegrees(parseFloat(longitude), parseFloat(latitude));
            ...
            viewer.entities.add({
                ...
                label: {
                    ...
                    pixelOffset: new Cesium.Cartesian2(0, -32), // 將標籤位置偏移，顯示在點的上方
                    heightReference: Cesium.HeightReference.CLAMP_TO_GROUND, // Ensures the label sticks to the ground
                    distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0.0, 1000000.0) // Visibility based on distance
